...
public class ImageUtil {
	...
	public static String generateThumbnail(ImageHolder thumbnail, String targetAddr){
		...
		try {
			...
			Thumbnails.of(thumbnail.getImage()).size(400, 400).toFile(descFile);
		} catch (IOException e) {
		
			throw new RuntimeException("水印图片创建失败"+descFile);
		}
		return relativePath;
	...
	private static String getRandomFileName() {
		...
		return random + nowtimeString+DateFormatUtil.getDateFormat(new Date());
	}
